...
export default function ExportTemplateTwo({ quoteDetails }: Props) {
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      if (quoteDetails.author.logoUrl) {
        const imageBase64 = await toBase64(quoteDetails.author.logoUrl);
      ...
      pdf.setFontSize(11);
      pdf.text(`${quoteDetails.author.companyName}`, 40, 130, {
        charSpace: -0.2,
      });
      pdf.setFont('Switzer-Regular', 'normal');
      pdf.setFontSize(10);
      pdf.text(`Contact Number: ${quoteDetails.author.contactNumber}`, 40, 141);
      pdf.text(`Email: ${quoteDetails.author.email}`, 40, 151);
      ...
      pdf.text(
        `Due Date: ${format(toDate(quoteDetails.validDate), 'dd MMM yyyy')}`,
        285,
        140
      );
      pdf.text(`Quote Number:  ${quoteNo.toUpperCase()}`, 285, 150);
      ...
      pdf.setFontSize(11);
      pdf.text(quoteDetails.billing.customerName, 40, 192);
      pdf.text(quoteDetails.billing.billingAddress, 40, 202);
      ...
      pdf.setGState(pdf.GState({ opacity: 0.5 }));
      pdf.setFontSize(10);
      pdf.text(`Discount (${quoteDetails.discount}%)`, 270, 395);
      pdf.setGState(pdf.GState({ opacity: 1 }));
      pdf.text(
        `${
      ...
      pdf.setGState(pdf.GState({ opacity: 0.5 }));
      pdf.setFontSize(10);
      pdf.text(`Tax (${quoteDetails.vat}%)`, 270, 410);
      pdf.setGState(pdf.GState({ opacity: 1 }));
      pdf.text(
        `${currencyFormatter(
      ...
      pdf.setGState(pdf.GState({ opacity: 1 }));
      pdf.text(`${currencyFormatter(String(quoteDetails.total))}`, 370, 435);
      ...
      pdf.text(`Account Holder: ${quoteDetails.author.companyName}`, 40, 495);
      pdf.text(
      ...
      pdf.text(`Bank: ${quoteDetails.author.companyBank}`, 40, 525);
      pdf.text(
