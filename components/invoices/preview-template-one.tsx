...
export default function PreviewInvoiceTemplateOne({
  invoiceDetails: { user },
}: Props) {
  const {
    invoiceNumber,
    ...
    <div className='bg-white p-10 rounded-md shadow-xs border-input border space-y-10 w-160.5'>
      <div className='w-full space-y-6'>
        <div className='flex flex-col items-start gap-1'>
          <h1 className='font-semibold text-3xl tracking-tight'>Invoice</h1>
        </div>
        <div className='flex flex-col items-start w-fit gap-1 text-sm ml-auto font-medium'>
          <p>
            {invoiceNumber !== 'null'
              ...
              : `Invoice Number: ${invoicePrefix}-XXXXX`}
          </p>
          <p>{`Date: ${format(new Date(), 'dd MMM yyyy')}`}</p>
          <p>
            {dueDate
              ? `Due Date: ${format(toDate(dueDate), 'dd MMM yyyy')}`
              : `Due Date: -`}
          </p>
        </div>
